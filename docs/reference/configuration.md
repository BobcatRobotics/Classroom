...
| `CODERUNNER_DOCKER_GID` | `0` | Gid owning the bind-mounted Docker socket, added to the non-root control process as a supplementary group (compose `group_add:`) so it can reach it. The `0` default matches Docker Desktop, whose socket is root-owned, so Docker Desktop needs no setting here. A Linux host running Docker Engine natively owns the socket by the `docker` group instead and must set this — find it with `stat -c '%g' /var/run/docker.sock` (stock Debian/Ubuntu installs often use `999`/`998`). |
