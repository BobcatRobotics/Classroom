...
| `CODERUNNER_DOCKER_GID` | `1001` | Host `docker` group gid, added to the non-root control process as a supplementary group (compose `group_add:`) so it can reach the bind-mounted socket. Find it with `stat -c '%g' /var/run/docker.sock` — stock Debian/Ubuntu installs often use `999`/`998`, not `1001`. Linux hosts only: compose also adds root unconditionally, which is what Docker Desktop's root-owned socket needs, so Docker Desktop needs no setting here. Do not set this to `0` — compose rejects duplicate `group_add` entries. |
