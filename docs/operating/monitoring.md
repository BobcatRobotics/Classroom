...
- High `container_memory_percent` approaching 100% — OOM risk; consider
  raising `CODE_MEMORY_LIMIT` or reducing the concurrency cap.
- High `active_workspaces` with slow build durations — CPU headroom is low;
  check the concurrency cap against host sizing (see [Capacity](./capacity.md)).
- `runs_total{terminal_status="failed"}` rising — check for build timeouts or
  container OOMs in the logs.
